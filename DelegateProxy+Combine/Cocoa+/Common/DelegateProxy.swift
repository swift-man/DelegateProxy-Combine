...
private final class MessageDispatcher {
  private var numberOfSubscriptions = [Int]()
  private let numberOfCounter: Publishers.SubscribersCounter<PassthroughSubject<[Any], Error>>
  private let dispatcher: PassthroughSubject<[Any], Error>
  ...
  init<P, D>(selector: Selector, delegateProxy _delegateProxy: DelegateProxy<P, D>) {
    ...
    let dispatcher = PassthroughSubject<[Any], Error>()
    self.dispatcher = dispatcher
    self.selector = selector
    
    self.result = dispatcher
      .handleEvents(receiveSubscription: { subscription in
        weakDelegateProxy?.checkSelectorIsObservable(selector); weakDelegateProxy?.reset()
    ...
    numberOfCounter = dispatcher.countingSubscribers()
  }
  
  func sendCompletion(_ completion: Subscribers.Completion<Error>) {
    self.dispatcher.send(completion: completion)
  }
  
  func send(_ input: [Any]) {
    self.dispatcher.send(input)
  }
  
  var hasObservers: Bool {
      return numberOfCounter.numberOfSubscribers > 0
  }
  
  func asObservable() -> AnyPublisher<[Any], Error> {
    return self.result
  }
}
